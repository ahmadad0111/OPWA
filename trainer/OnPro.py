...
from OPE_loss import OPELoss

class OnProTrainer(object):
        ...
        for subject_id in range(self.args.start_range, self.args.subject_range):
            ...
            for episode in range(1, self.args.max_episode + 1):
                ...
                for i, batch in enumerate(tqdm_gen, 1):
                    ...
                    # loss on on stream data
                    embeddings = self.model.eegnet(data)
                    data_aug = self.sim_aug(data)
                    embeddings_aug = self.model.eegnet(data_aug)

                    rot_sim_labels = torch.cat([labeled],dim=0)
                    simclr = normalize(torch.cat([embeddings, embeddings_aug], dim=0))
                    sim_matrix = get_similarity_matrix(simclr)

                    loss_sim = Supervised_NT_xent_n(sim_matrix, labels=rot_sim_labels, temperature=0.1)

                    logits= self.model(data)

                    loss_OPE, cur_new_proto_z, cur_new_proto_zt = self.OPELoss(embeddings, embeddings_aug, labeled, subject_id, True)

                    # loss on buffer data
                    buf_embeddings = self.model.eegnet(buf_data)
                    buf_data_aug = self.sim_aug(buf_data)
                    buf_embeddings_aug = self.model.eegnet(buf_data_aug)

                    buf_rot_sim_labels = torch.cat([buf_labels],dim=0)
                    buf_simclr = normalize(torch.cat([buf_embeddings, buf_embeddings_aug], dim=0))
                    buf_sim_matrix = get_similarity_matrix(buf_simclr)

                    buf_loss_sim = Supervised_NT_xent_n(buf_sim_matrix, labels=buf_rot_sim_labels, temperature=0.1)

                    buf_logits= self.model(buf_data)

                    buf_loss_OPE, buf_new_proto_z, buf_new_proto_zt = self.OPELoss(buf_embeddings, buf_embeddings_aug, buf_labels, subject_id, True)

                    loss = F.cross_entropy(buf_logits,buf_labels) + loss_sim + loss_OPE + buf_loss_sim + buf_loss_OPE


                    #loss,acc = self.proto(embeddings, label_comb, update_prototypes=True)
            ...
            # if subject_id == 12 or subject_id == 24 or subject_id == 36:
